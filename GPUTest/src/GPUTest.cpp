...
void loadImage(GPUTexture & image, const std::string & name)
{
    ...
    Img<Eigen::Matrix<unsigned char, 3, 1>> imageRaw(720, 1280, (Eigen::Matrix<unsigned char, 3, 1> *)img.ptr);
...
void loadDepth(GPUTexture & depth, const std::string & name)
{
    ...
    Img<unsigned short> depthRaw(720, 1280, (unsigned short *)img.ptr);
...
void loadVertices(GPUTexture & vertices, GPUTexture & normals, const std::string & name)
{
    ...
    Img<Eigen::Vector4f> verts(720, 1280);
    Img<Eigen::Vector4f> norms(720, 1280);

    Eigen::Vector4f point(0, 0, 0, 1);
    Eigen::Vector4f norm(0, 0, 0, 1);

    for(unsigned int row = 1; row < 720 - 1; row++)
    {
        for(unsigned int col = 1; col < 1280 - 1; col++)
        {
...
int main(int argc, char * argv[])
{
    ...
    pangolin::CreateWindowAndBind("GPUTest", 1280, 720);
    pangolin::Display("Image").SetAspect(1280.0f / 720.0f);

    GPUTexture firstImage(1280, 720,
                          GL_RGBA,
                          GL_RGB,
                          GL_UNSIGNED_BYTE,
                          false, true);
    ...
    GPUTexture secondImage(1280, 720,
                           GL_RGBA,
                           GL_RGB,
                           GL_UNSIGNED_BYTE,
                           false, true);
    ...
    GPUTexture secondDepth(1280, 720,
    ...
    GPUTexture vertexTexture(1280, 720,
                             GL_RGBA32F,
                             GL_LUMINANCE,
                             GL_FLOAT,
                             false, true);

    GPUTexture normalTexture(1280, 720,
                             GL_RGBA32F,
                             GL_LUMINANCE,
                             GL_FLOAT,
                             false, true);
    ...
    RGBDOdometry odom(1280, 720, K(0, 2), K(1, 2), K(0, 0), K(1, 1));
